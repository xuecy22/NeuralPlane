...
class UnreachPosture(BaseTerminationCondition):
    """
    UnreachPosture
    End up the simulation if the aircraft didn't reach the target posture in limited time.
    """
    ...
    def get_termination(self, task, env, info={}):
        ...
        # mask4 = torch.abs(altitude - env.target_altitude) >= 100
        # 判断是否到达target_pitch
        mask4 = torch.abs(pitch - task.target_pitch) >= torch.pi / 36
        # 判断是否到达target_vt
        ...
        if torch.any(bad_done):
            self.log(f'unreach posture!')
            print(torch.sum(bad_done), 'unreach posture!')
        if torch.any(done):
