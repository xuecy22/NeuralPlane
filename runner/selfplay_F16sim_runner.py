...
from F16sim_runner import F16SimRunner
...
class SelfplayJSBSimRunner(F16SimRunner):
    ...
    def eval(self, total_num_steps):
        ...
        while total_episodes < self.eval_episodes:
            ...
            obs, eval_rewards, dones, bad_dones, exceed_time_limits, eval_infos = self.eval_envs.step(actions)
            ...
            rnn_states[reset_env == True] = np.zeros(((reset_env == True).sum(), *rnn_states.shape[1:]), dtype=np.float32)

            opponent_masks[dones_env == True] = \
                np.zeros(((dones_env == True).sum(), *opponent_masks.shape[1:]), dtype=np.float32)
            opponent_rnn_states[reset_env == True] = \
                np.zeros(((reset_env == True).sum(), *opponent_rnn_states.shape[1:]), dtype=np.float32)

            # [Selfplay] Get rewards
            opponent_rewards = eval_rewards[:, self.num_agents//2:, ...]
            opponent_cumulative_rewards += opponent_rewards
            opponent_episode_rewards.append(opponent_cumulative_rewards[reset_env == True])
            opponent_cumulative_rewards[reset_env == True] = 0

            eval_rewards = eval_rewards[:, :self.num_agents // 2, ...]
            cumulative_rewards += eval_rewards
            episode_rewards.append(cumulative_rewards[reset_env == True])
            cumulative_rewards[reset_env == True] = 0
